...
class GreedyWorker():
    ...
    def evaluate_initial(self):
        ...
        vvp_command = [self.path['vvp'], '-l', self.modulename+'.log', self.modulename+'.iv']
        # with open(output_truth, 'w') as f:
            # subprocess.call(vvp_command, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        with open(output_truth, 'w') as f:
            subprocess.call([self.path['vvp'], self.modulename+'.iv'], stdout=f)
        
        #BUG: 2023-12-12: When use adder128.v, .truth generated by iverilog tool is full of 'z'
        # => Fixed in 2023-12-18: It is majorly the problem of the original verilog files
        ...
        while len(partitioned) > 0:
            ...
            if inp > 14 or out > 6:
                lsoracle_command = 'read_verilog ' + mod_path + '; ' \
                        'partitioning 2 -c ' + self.path['part_config'] + '; ' \
                        'get_all_partitions ' + part_dir
                with open(log_partition, 'a') as file_handler:
                    subprocess.call([self.path['lsoracle'], '-c', lsoracle_command], stderr=subprocess.STDOUT, stdout=file_handler)
                
                # BUG: 2024-1-4, partitioned list is extended 3 new partitions in default =>
                # However, LSOracle could generate possibly two partitions, with 1 partition contains 0 Logic Cones
                # This will cause the top file error in linking submodules => seems like the error of LSOracle Tool
                # Solution => directly change 3 partitions as 2 partitions.
                partitioned.extend([mod+'_'+str(i) for i in range(2)])
                # print(partitioned)
    ...
    def next_iter(self, parallel, cpu_count, step_size, threshold=[1000000.], least_error=False, track=3, accel=0):
        ...
        print('Log: Current stream of factorization degree: ','\n'.join(map(str, self.curr_streams)))
